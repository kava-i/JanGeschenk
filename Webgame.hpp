  ...
  public:
    ...
    ~Webgame() {
	  ...
	  context_ptr on_tls_init(websocketpp::connection_hdl hdl) {
	    ...
	    try {
            ...
            const char* chain_file = std::getenv("CERT_CHAIN_FILE");
            const char* privkey = std::getenv("CERT_PRIV_KEY");
            std::cout << "About to use chain-file at: " << chain_file << std::endl;
            std::cout << "About to use priv-key at: " << privkey << std::endl;
		    ctx->set_password_callback(bind(&get_password));
            ctx->use_certificate_chain_file(chain_file);
            ctx->use_private_key_file(privkey, asio::ssl::context::pem);
    ...
    void stop() {
	  ...
	  void run() {
        ...
        #ifdef _WSS_
          echo_server.set_tls_init_handler(bind(&Webgame<GameType, Game>::on_tls_init,this,::_1));
        #endif
